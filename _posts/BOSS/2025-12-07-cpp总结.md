---
layout:     post
title:      CPP 总结
subtitle:   
date:       2025-12-07
author:     phoenixZ
header-img: /img/oip4.jpg
catalog: true
tags:
    - boss
    - cpp
---
# 🚀关键字

## 💡static

1. 局部变量加static， 存储在全局区域，函数返回后它的值不会改变
2. static 修饰函数限制函数的可见行，以及避免命名冲突
3. 如果修饰类的成员函数， 不依赖对象，不能访问普通成员变量，只能访问static修饰的成员变量

# 🚀智能指针

## 💡shared_ptr

### 原理

- 引用计数，当shared_ptr被拷贝(增加拥有者)或销毁时，计数才改变
- 每当有一个新的实例指向这个对象的时候引用计数+1
- 每当一个shared_ptr被销毁（比如离开了作用阈）或重置时，引用计数-1
- 引用计数清零的时候，调用该对象的析构函数自动释放被管理对象占用的内存
- 循环引用的问题: 两个对象互相持有对方的shared_ptr, 计数无法清零，从而引发内存泄漏的问题

### shared_ptr是线程安全的吗

1. 引用计数是线程安全的
2. shared_ptr对象本身不是线程安全的
3. 指向的资源不是线程安全的

# 🚀类相关

## 💡析构函数与构造函数

### 执行顺序

1. 构造函数

- **基类构造函数** ：按照继承列表中的声明顺序执行
- **成员对象构造函数** ：按照类定义中成员变量的声明顺序执行
- **派生类自身的构造函数体**

2. 析构函数

- **派生类自身的析构函数体**
- **成员对象析构函数**
- **基类析构函数**

> **重要提醒** ：在多继承或深层继承中，这种“对称性”确保了派生类在销毁时，其依赖的底层基类资源依然有效，直到最后一步才被释放。

### 可以是虚函数吗

1. 构造函数不能是虚函数：如果是虚函数，就需要用虚函数指针去查找虚函数表，但是虚函数指针是在构造函数执行过程中才被初始化；这样虚函数指针是个随机值或者未分配;
2. 析构函数是虚函数：然后先子类再父类的释放顺序，实现完整销毁

## 💡多态

### 核心组件

1. 虚函数表： 编译器为每一个拥有虚函数的类创建一个静态表格，存放该类所有虚函数的地址
2. 虚函数指针： 编译器在类的每个对象实例中，隐式添加一个指针(通常位于对象内存布局的最开头)， 指向虚函数表

### 三个阶段

1. 编译器(建立虚函数表)
2. 对象创建期(关联虚函数表)
3. 运行期(动态寻址)

# 🚀STL

## 💡Vector

# 🚀算法

## 💡A_star

{% highlight cpp %}
    struct Node
    {
        int x, y; //坐标
        long long g_score, f_score;

    Node *parent;

    Node(int x1, int y1)
            : x(x1), y(y1)
        {
            g_score = numeric_limits`<long long>`::max();
            f_score = numeric_limits `<long long>`::max();
        }
    };

    using PQElement = pair<long long, Node *>;

    int Hfunction(Node *A, Node *B)
    {
        return abs(A->x - B->x) + abs(A->y - B->y);
    }

    void AStar(vector<vector`<int>`> &map, pair<int, int> start, pair<int, int> end, vector<pair<int, int>> &path)
    {
        int R = map.size(), C = map[0].size();

    vector<vector`<Node>`> nodes(R);

    for (int i = 0; i < R; i++)
        {
            nodes[i].reserve(C);

    for (int j = 0; j < C; j++)
            {
                nodes[i].emplace_back(i, j);
            }
        }

    priority_queue<PQElement, vector`<PQElement>`, greater `<PQElement>`> pq;

    auto start_node = &nodes[start.first][start.second];
        auto target_node = &nodes[end.first][end.second];

    start_node->g_score = 0;
        start_node->f_score = Hfunction(start_node, target_node);

    pq.push({start_node->f_score, start_node});

    while (!pq.empty())
        {
            long long U = pq.top().first;
            auto V = pq.top().second;

    if (U > V->f_score) continue;

    pq.pop();

    if (V == target_node)
            {
                auto tmp = V;
                while (tmp != nullptr)
                {
                    path.push_back({tmp->x, tmp->y});
                    if (tmp == start_node) break;
                    tmp = tmp->parent;
                }

    reverse(path.begin(), path.end());
                return;
            }

    int dr[4] = {-1, 1, 0, 0};
            int dc[4] = {0, 0, -1, 1};

    for (int i = 0; i < 4; i++)
            {
                int r = V->x + dr[i];
                int c = V->y + dc[i];

    if (r < 0 || r >= R || c < 0 || c >= C) continue;

    auto neighbor = &nodes[r][c];

    if (V->g_score + 1 < neighbor->g_score)
                {
                    neighbor->g_score = V->g_score + 1;
                    neighbor->f_score = V->g_score + 1 + Hfunction(neighbor, target_node);
                    neighbor->parent = V;
                    pq.push({neighbor->f_score, neighbor});
                }
            }
        }
    }

{% endhighlight cpp %}

## 💡排序

### 冒泡排序

{% highlight cpp %}
    #include `<iostream>`
    #include `<vector>`

    using namespace std;

    void bubbleSort(vector`<int>` &arr)
    {
        int n = arr.size();

    for (int i = 0; i < n - 1; ++i)
        {
            bool swapped = false;

    for (int j = 0; j < n - 1 - i; ++j)
            {
                if (arr[j] > arr[j + 1])
                {
                    swap(arr[j], arr[j + 1]);
                    swapped = true;
                }
            }

    if (!swapped)
                break;
        }
    }

{% endhighlight cpp %}

### 快速排序

{% highlight cpp %}
    int findPartition(vector `<int>` &arr, int low, int high)
    {
        int base = arr[high]; // 选最右边的作为基准

    int i = low - 1;

    for (int j = low; j < high; ++j)
        {
            if (arr[j] <= base)
            {
                ++i;
                swap(arr[i], arr[j]);
            }
        }

    swap(arr[i + 1], arr[high]);

    return i + 1;
    }

    void quickSort(vector`<int>` &arr, int low, int high)
    {
        if (low < high)
        {
            int idx = findPartition(arr, low, high);
            quickSort(arr, low, idx - 1);
            quickSort(arr, idx + 1, high);
        }
    }
{% endhighlight cpp %}

## 💡二分查找

{% highlight cpp %}
    int binaryFind(vector `<int>` &arr, int left, int right, int target)
    {
        while (left <= right)
        {
            int mid = left + (right - left) / 2;

    if (arr[mid] < target)
            {
                left = mid + 1;
            }

    else if (arr[mid] > target)
            {
                right = mid - 1;
            }

    else
            {
                return mid;
            }
        }

    return -1;
    }
{% endhighlight cpp %}

# 🚀新特性

## 💡function

### 解决什么问题？

是一个通用的函数包装器，用于统一处理各种可调用对象
可调用对象包括：

1. 普通函数
2. 仿函数
3. lambda表达式
4. 类的成员函数

### 底层原理

std::function的实现依赖类型擦除，主要通过以下方式实现多态

- 抽象基类+虚函数：内部定义一个抽象基类，为每个具体调用的类型生成派生类模板，存储实际对象并实现调用、复制、析构
- 手动虚表：避免RTTI开销，使用函数指针数组模拟虚表，管理调用、复制、析构等操作
- 内存管理：小对象优化(SBO)减少堆分配

简单理解：利用类型擦除+函数指针/虚表实现的一种运行时多态

**类型擦除**：用于隐藏对象的具体类型信息，从而让不同类型的对象可以通过统一的接口来操作
**RTTI**： 是运行时类型信息的缩写

### 代码示例

{% highlight cpp %}
#include `<iostream>`
#include `<functional>` //配套的头文件

int add(int a, int b)
{
    return a + b;
}

struct Test
{
    int operator()(int a, int b)
    {
        return a * b;
    }
};

class Robot
{
public:
    void sayHello(std::string name)
    {
        std::cout << "Robot says: Hello, " << name << std::endl;
    }
};

void TestCB(int a, std::function<void(bool)> callback)
{
    bool success = true;
    if (callback)
    {
        callback(success);
    }
}

int main()
{
    std::function<int(int, int)> func;

    //! 情形1： 包装普通函数
    func = add;
    std::cout << "add : " << func(2, 3) << std::endl;

    //! 情形2： 包装仿函数
    func = Test();
    std::cout << "Test: " << func(1, 2) << std::endl;

    //! 情形3： 包装lambda表达式
    func = [](int a, int b) { return a - b; };
    std::cout << "lambda: " << func(33, 24) << std::endl;

    //! 情形4： 包装类的成员函数
    Robot robot;
    std::function[void(std::string)](void(std::string)) func1 = std::bind(&Robot::sayHello, &robot, std::placeholders::_1);
    func1("test function");

    //! 情形5：实现回调函数
    TestCB(20, [](bool tmp) { std::cout << tmp << std::endl; });

    return 0;
}
{% endhighlight cpp %}

## 💡bind绑定器

### 解决什么问题

将函数对象与参数绑定在一起形成新的可调用对象，便于简单调用

### 底层原理

将参数存储在内部的tuple中，并通过模板元变成在调用时进行"参数重组"

### 用法示例

### 代码示例

{% highlight cpp %}
#include `<functional>`
#include `<iostream>`
#include `<memory>`
#include `<string>`
#include `<vector>`

using namespace std;
using namespace std::placeholders;

void Test(int a)
{
    cout << __func__ << "->" << a << endl;
}

class Person {
private:
public:
    int m_a;
    Person() = default;
    Person(int a)
        : m_a(a)
    {
    }

    int operator()(int b)
    {

    return m_a + b;
    }

    void show(int a)
    {
        cout <<__func__ << "->" << a << endl;
    }
};

int main()
{
    Person p1;

    //绑定普通函数
    auto f1 = bind(Test, _1);  //_1代表占第一位
    f1(11);

    //绑定成员函数
    auto f2 = bind(&Person::show, &p1, 2);
    f2(2);

    //绑定成员变量（public）
    auto f3 = bind(&Person::m_a, &p1);
    f3() = 111;
    cout << p1.m_a << endl;

    return 0;
}
{% endhighlight cpp %}

## 💡lamda表达式

### 底层原理

底层实现是生成未命名类的实例，重载operator().

### 作用

1. 就地定义： 不需要为了一个简单的逻辑单独写一个函数， 使代码更加紧凑
2. 闭包能力： 能捕获上下文中的变量，在处理回调、异步操作、多线程时非常强大
3. 代码可读性： 逻辑与使用位置一致，减少阅读代码的成本

### 代码示例

{% highlight cpp %}
#include `<iostream>`
#include `<vector>`
#include `<algorithm>`
#include `<functional>`
#include `<map>`
#include `<string>`
#include `<thread>`
#include `<chrono>`

// 机器人系统类：演示如何在成员函数中使用 Lambda 和 this
class RobotSystem {
public:
    RobotSystem(std::string name) : robot_name_(name), battery_level_(100) {}

    // 1. 成员函数内的 Lambda：必须捕获 [this] 才能访问类成员
    void startSelfCheck() {
        std::cout << "[System] Starting self-check for " << robot_name_ << "...\n";

    auto check_battery =[this]() {
            // 访问私有变量 battery_level_
            std::cout << "[Check] Battery is at " << this->battery_level_ << "%" << std::endl;
            if (this->battery_level_ > 20) {
                this->logStatus("Battery OK");
            }
        };
        check_battery();
    }

    void logStatus(std::string status) {
        std::cout << "[Log] " << status << std::endl;
    }

private:
    std::string robot_name_;
    int battery_level_;
};

int main() {
    // --- 示例 1: 基础用法 (用于 STL 算法) ---
    std::vector`<double>` sensor_readings = {1.2, 0.5, 3.8, 2.4};

    // 使用 Lambda 进行降序排序
    std::sort(sensor_readings.begin(), sensor_readings.end(), [](double a, double b) {
        return a > b;
    });

    std::cout << "Sorted Readings: ";
    // 使用 Lambda 打印
    std::for_each(sensor_readings.begin(), sensor_readings.end(), [](double d) {
        std::cout << d << " ";
    });
    std::cout << "\n\n";

    // --- 示例 2: 闭包捕获 (值捕获与引用捕获) ---
    double offset = 0.5;
    double sum = 0;

    // [offset] 是值捕获：拷贝一份副本，内部不可修改外部的 offset
    // [&sum]   是引用捕获：内部直接修改外部变量 sum 的值
    std::for_each(sensor_readings.begin(), sensor_readings.end(), [offset, &sum](double val) {
        sum += (val + offset);
    });
    std::cout << "Sum with offset: " << sum << "\n\n";

    // --- 示例 3: 在类内使用 (捕获 this) ---
    RobotSystem myRobot("Standard-Bot");
    myRobot.startSelfCheck();
    std::cout << "\n";

    // --- 示例 4: 结合 std::function 实现指令回调 (机器人开发常用) ---
    std::map<std::string, std::function<void(int)>> command_dispatcher;

    // 动态注册不同的 Lambda 指令
    command_dispatcher["move_forward"] = [](int power) {
        std::cout << "Action: Moving forward with power " << power << std::endl;
    };
    command_dispatcher["rotate"] = [](int angle) {
        std::cout << "Action: Rotating " << angle << " degrees." << std::endl;
    };

    // 模拟接收到外部指令
    std::string incoming_cmd = "move_forward";
    if (command_dispatcher.count(incoming_cmd)) {
        command_dispatcher[incoming_cmd](80); // 执行对应的 Lambda
    }

    // --- 示例 5: mutable 关键字 ---
    int counter = 10;
    // 默认值捕获不可修改副本，加上 mutable 后允许在 Lambda 内部修改副本内容
    auto tick =[counter]() mutable {
        counter++;
        std::cout << "Internal counter (mutable copy): " << counter << std::endl;
    };
    tick();
    std::cout << "External counter (original): " << counter << " (unchanged)\n";

    return 0;
}
{% endhighlight cpp %}

## 💡移动构造

### 作用

1. 资源转移而非拷贝：当一个对象即将被销毁时，移动构造可以转移它的资源，而不是重新分配并复制一份
2. 性能提升： 将O(n)的拷贝操作降低为O(1)的指针交换
3. 支持仅可移动的类型： 如unique_ptr 或 thread

### 代码示例

{% highlight cpp %}
class RobotData {
public:
    double* data;
    size_t size;

    RobotData(size_t s) : size(s) {
        data = new double[s];
        std::cout << "Allocated memory at " << data << std::endl;
    }

    // 移动构造函数
    // 接收一个右值引用 RobotData&&
    RobotData(RobotData&& other) noexcept {
        this->data = other.data;  // “偷”走指针
        this->size = other.size;

    other.data = nullptr;     // 将源对象置空，防止析构时释放内存
        other.size = 0;
        std::cout << "Moved memory to new object" << std::endl;
    }

    ~RobotData() {
        delete[] data;
    }

    // 禁止拷贝构造（为了演示移动）
    RobotData(const RobotData&) = delete;
};

int main() {
    std::vector`<RobotData>` vec;
    // 此时会调用移动构造函数，而不是昂贵的拷贝
    vec.push_back(RobotData(1000));
    return 0;
}
{% endhighlight cpp %}

## 💡完美转发

### 作用

1. 保持属性不变：在编写模板函数时，将参数原封不动地传递给内部函数。
2. 解决痛点：如果参数是左值，传给内部也是左值；如果是右值，传给内部依然是右值
3. 实现基础：依赖于 万能引用（Universal Reference, T&&） 和 std::forward`<T>`

### 代码示例

{% highlight cpp %}
#include `<iostream>`
#include `<utility>`

void process(int& x) { std::cout << "Lvalue processed\n"; }
void process(int&& x) { std::cout << "Rvalue processed\n"; }

// 完美转发包装器
template`<typename T>`
void logAndProcess(T&& arg) {
    std::cout << "Logging... ";
    // std::forward`<T>`(arg) 会根据 T 的类型决定转发左值还是右值
    process(std::forward`<T>`(arg));
}

int main() {
    int a = 10;
    logAndProcess(a);  // 传入左值 -> 调用 process(int&)
    logAndProcess(20); // 传入右值 -> 调用 process(int&&)
    return 0;
}
{% endhighlight cpp %}

# 🚀内存管理

# 🚀操作系统

## 💡进程和线程

### 进程和线程的区别

1. 基本概念
   - 进程是系统进行资源分配和调度的基本单位，拥有独立的内存空间地址
   - 线程是CPU进行调度和分配的，共享所属进程的资源
2. 区别
   - 进程拥有资源，线程不拥有资源
   - 进程间通信开销大，线程间通信简单
   - 一个进程崩了不会影响其他线程，但是一个线程崩了整个进程可能都会退出

### 进程和线程的通信方式

1. 进程通信方式
   - 管道：半双工
   - 信号：简单的异步通知机制
   - 消息队列：克服了信号载荷少、管道无格式的缺点
   - 共享内存：最快的方式，但需要注意同步
   - 套接字(socket)：可跨机器通信
2. 线程通信方式
   - 共享全局变量
   - 信号量
   - 条件变量
   - 套接字

## 💡锁

### 死锁

1. 产生原因？
   - 两个或者多个进程在执行过程中，因争夺资源而造成的一种互相等待的现象
2. 死锁产生的必要条件
   - 互斥条件：资源同一时间只能被一个进程占用
   - 请求与保持： 已占有资源，同时请求新资源
   - 不剥夺条件： 资源不能被强制抢占
   - 循环等待： 形成了一个进程等待资源的环路
3. 解决死锁
   - 破坏四个必要条件之一
   - 分配资源前进行风险评估； 银行家算法
   - 强行剥夺资源、杀死进程

### 典型的锁

- 互斥锁(mutex)：如果锁被占用，线程会进入睡眠，交出CPU给其他线程。等锁释放后被唤醒； 用于等待时间长，不希望浪费CPU的场景；
- 自旋锁(spinlock)：如果锁被占用，线程会循环忙等(cpu不停空转)，直到拿到锁；用于等待时间短的场景，因为线程切换的开销比忙等还要大；
